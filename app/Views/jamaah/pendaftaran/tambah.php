            ...
            <div class="col-12">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <div style="margin-bottom: 10px;">
                                    <span>
                                        Batas Jamaah : <?= $kloter['batas_jamaah']; ?> Jamaah
                                    </span>
                                </div>
                                <div style="margin-bottom: 10px;">
                                ...
                                <div style="margin-bottom: 10px;">
                                    <span>Kloter : <span><?= $kloter['nama'];  ?></span>
                                    </span>
                                </div>
                                <div style="margin-bottom: 10px;">
                                    <span>Biaya : <span><?= "Rp." . number_format($paket['biaya'], 0);  ?></span>
                                    </span>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div style="margin-bottom: 10px;">
                                    <span>Tanggal Berangkat : <?= date("d, F Y",strtotime($paket['tgl_berangkat']))  ?></span>
                                </div>
                                <div style="margin-bottom: 10px;">
                                <span>Tanggal Kepulangan : <?= date("d, F Y",strtotime($paket['tgl_pulang']))  ?></span>
                                </div>
                                <div style="margin-bottom: 10px;">
                                    <span>Provider : <?= $paket['provider'];  ?></span>
                                </div>
                                <div style="margin-bottom: 10px;">
                                    <span>Asuransi : <span><?= $paket['asuransi'];  ?></span>
                                    </span>
                                </div>
                                <?php
                                        $banks = $rekening_penampung->where('id',$paket['rekening_penampung_id'])->first();
                                ?>
                                <div style="margin-bottom: 10px;">
                                    <span>Rekening Penampung : <span><?= $banks['no_rekening']  . ' / ' . $banks['nama'] ?></span>
                                    </span>
                                </div>
                            </div>
                        </div>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <a title="Kembali" href="<?= base_url("detail_pendaftaran_kloter/" . $id);  ?>" class="btn btn-warning"><i class="fas fa-arrow-left"></i></a>
                        <?php if ($count) : ?>
                        ...
                        <?php endif; ?>
                        <?php if ($paket['keberangkatan'] != "sudah") : ?>
                            <a href="<?= base_url("insert_jamaah/$id/$id_kloter"); ?>" class="btn btn-primary "><i class="fas fa-plus"></i></a>
                        <?php endif; ?>
                                ...
                                <tbody>
                                    ...
                                    foreach ($result as $row) : ?>
                                        <tr>
                                            ...
                                            <td>
                                                <a href="<?= base_url('detail_jamaah/' . $kloter['id'] . '/' . $id . '/' . $row['id']); ?>" class="btn btn-primary" title="Detail" ><i class="fa fa-eye"></i></a>
                                                <?php if ($paket['keberangkatan'] != "sudah") : ?>
                                                    <?php if ($row['selesai_pembayaran'] != "sudah" && $row['status_bayar'] != "lunas") : ?>
                                                        <a href="#" title="Hapus" class="btn btn-danger" data-toggle="modal" data-target="#hapus<?= $row['id'] ?>"><i class="fa fa-trash"></i></a>
                                                    <?php endif; ?>
                                                    <?php if (empty($row['user_id'])) : ?>
                                                        <a href="<?= base_url("edits_jamaah/" . $id_kloter   . '/' . $id . '/' .  $row['id']); ?>" title="Edit" class="btn btn-success"><i class="fa fa-pen"></i></a>
                                                    <?php endif; ?>
                                                    <?php if ($row['selesai_pembayaran'] != "sudah" && $row['status_bayar'] != "lunas" && $row['status_bayar'] != "cicil" && $row['status_bayar'] != "DP") : ?>
                                                        <a href="<?= base_url("pindah_paket_umrah/$row[id]/$id_kloter/$id");  ?>" title="Pindah Travel" class="btn btn-warning"><i class="fa fa-edit"></i></a>
                                                    <?php endif; ?>
                                                    <?php if (!empty($row['user_id'])) : ?>
                                                        <?php if (empty($row['selesai_pembayaran']) && $row['status_bayar'] == "lunas") : ?>
                                                        <?php endif; ?>
                                                        <?php if ($row['selesai_pembayaran'] != "sudah" && $row['status_bayar'] != "lunas") : ?>
                                                            <a href="<?= base_url("pembayaran" . '/' . $row['id'] . '/' . $id . '/' . $id_kloter);  ?>" class="btn btn-info" title="Pembayaran"><i class="fa fa-clipboard"></i></a>
                                                        <?php endif; ?>
                                                    <?php endif; ?>
                                                    <?php if (empty($row['user_id'])) : ?>
                                                        <a href="<?= base_url("kelengkapan_jamaah/$row[id]/$id/$id_kloter"); ?>" class="btn btn-primary " title="Kelengkapan"><i class="fa fa-address-card"></i></a>
                                                    <?php endif; ?>
                                                <?php endif; ?>
                                            </td>
                                        </tr>
...
<div class="modal fade" tabindex="-1" role="dialog" id="tambah">
    <div class="modal-dialog modal-lg" role="document">
        <form method="POST" enctype="multipart/form-data" action="<?= base_url("tambah_jamaah");  ?>" class="modal-content">
            <input type="hidden" name="id_paket" value="<?= $id;  ?>">
            <input type="hidden" name="id_kloter" value="<?= $id_kloter;  ?>">
            <div class="modal-header">
...
<?php foreach ($result as $main) : ?>
    <div class="modal fade" tabindex="-1" role="dialog" id="vaksin<?= $main['id'] ?>">
        <div class="modal-dialog modal-lg" role="document">
            <form method="POST" enctype="multipart/form-data" action="<?= base_url("vaksin/" . $main['id']);  ?>" class="modal-content">
                <div class="modal-header">
                    <input type="hidden" name="id_paket" value="<?= $id;  ?>">
                    <input type="hidden" name="id_kloter" value="<?= $id_kloter;  ?>">
            ...
            <form method="POST" enctype="multipart/form-data" action="<?= base_url("bayar/" . $main['id']);  ?>" class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-1">
                        ...
                        <select name="rekening" class="form-control" required autocomplete="off" id="">
                            ...
                            <?php foreach ($bank as $banks) : ?>
                                <option value="<?= $banks['id']  ?>" <?= ($banks['id'] == $main['rekening_penampung']) ? "selected" : "";  ?>>
                    ...
                    <div class="mb-1">
                        ...
                        <select name="status" class="form-control" required id="pembayaran_baru<?= $main['id'] ?>" onchange="pembayaran()">
                            <option value="">Pilih</option>
    ...
    <div class="modal fade" tabindex="-1" role="dialog" id="edit<?= $main['id'] ?>">
        <div class="modal-dialog modal-lg" role="document">
            <form method="POST" enctype="multipart/form-data" action="<?= base_url("edit_jamaah/" . $main['id']);  ?>" class="modal-content">
                <input type="hidden" name="id_paket" value="<?= $id;  ?>">
                <input type="hidden" name="id_kloter" value="<?= $id_kloter;  ?>">
                <div class="modal-header">
                            ...
                            <div class="mb-3">
                                ...
                                <select name="provider" class="form-control" required id="">
                                    ...
                                    <?php foreach ($provider as $providert) : ?>
                                        <option <?= ($providert['nama_provider'] == $main['provider']) ? "selected" : "";  ?> value="<?= $providert['nama_provider'];  ?>"><?= $providert['nama_provider'];  ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            <div class="mb-3">
                                ...
                                <select name="asuransi" class="form-control" required id="">
                                    ...
                                    <?php foreach ($asuransi as $main_dua) : ?>
                                        <option value="<?= $main_dua['nama'];  ?>" <?= ($main_dua['nama'] == $main['asuransi']) ? "selected" : "";  ?>>
