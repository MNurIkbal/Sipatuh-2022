...
class KeberangkatanController extends BaseController
{
    ...
    public function tambah_keberangkatan_real()
    {
        if (!session()->get("login") || session()->get("login") == null) {
            return redirect()->to("/");
            exit;
        ...
        // }
        // cara membuat kondisi
        $id_klo = $this->request->getVar('id_kloter');
        try {
            $pakets_baru = new PaketModel();
            $paket_tes = $pakets_baru->where('id', $this->request->getVar("id"))->first();
            $time_waktu = $this->request->getVar('tgl_berangkat');
            ...
            return redirect()->to("detail_realisasi/". $id_klo . '/' . $this->request->getVar("id"))->with("success", "Data Berhasil Ditambahkan");
        } catch (\Throwable $th) {
            return redirect()->to("detail_realisasi/" . $id_klo . '/' . $this->request->getVar("id"))->with("error", "Data Gagal Ditambahkan");
            //throw $th;
        }
    }
    ...
    public function edit_keberangkatan_real()
    {
        if (!session()->get("login") || session()->get("login") == null) {
            return redirect()->to("/");
            exit;
        ...
        return redirect()->to("detail_realisasi/" . $id_klo . '/' . $this->request->getVar("id_paket"))->with("success", "Data Berhasil Diupdate");
    }
